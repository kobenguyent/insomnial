...
Insomniall is a desktop application built on top of [Electron](http://electronjs.org/). Electron provides a Chromium runtime for the Insomnial web app to run inside, as well as additional tools to provide access to operating system features.
...
- [`Electron Builder`](https://github.com/electron-userland/electron-builder) is used to help build, sign, and package Insomnial for distribution.
- [`libcurl`](https://curl.se/libcurl/) is the library that Insomnial uses to make requests. We used libcurl as our HTTP client of choice because it allows the deepest amount of debuggability and control of HTTP requests.
...
Insomnial uses [`npm workspaces`](https://docs.npmjs.com/cli/v9/using-npm/workspaces?v=true) to manage multiple npm packages within a single repository. There are currently the following package locations:

- `/packages` contains related packages that are consumed by `Insomnial` or externally.

## The `Insomnial` Main Package

`/packages/Insomnial` is the entry point for the app. All other packages are imported from this one.
...
Insomnial stores data in a few places:
...
The structure for smoke tests is explained in the smoke testing package: [`packages/Insomnia-smoke-test`](packages/Insomnial-smoke-test).

## Technical Debt

This is just a brief summary of Insomnial's current technical debt.
...
- `.npmrc`
- `.nvmrc`
- `packages/Insomnial/package.json` electron and node-libcurl
