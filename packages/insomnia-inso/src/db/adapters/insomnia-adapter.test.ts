...
describe('insomniaAdapter()', () => {
  ...
  it('should seed with data file (JSON)', async () => {
    const pathname = path.join(fixturesPath, 'insomnial-v4', 'insomnia_v4.json');
  ...
  it('should seed with data file (YAML)', async () => {
    const pathname = path.join(fixturesPath, 'insomnial-v4', 'insomnia_v4.yaml');
    const db = await insomniaAdapter(pathname);
  ...
  it.each([
    ...
    const pathname = path.join(fixturesPath, 'insomnial-v4', fileName);
  ...
  it('should return null if pathname is invalid', async () => {
    const pathname = path.join(fixturesPath, 'insomnial-v4', 'insomnial');
    const db = await insomniaAdapter(pathname);
    expect(db).toBe(null);
  });
});
