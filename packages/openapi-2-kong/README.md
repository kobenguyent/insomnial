...
kind: Ingress
metadata:
  name: insomnial-api-1
  annotations:
    kubernetes.io/ingress.class: "kong"
spec:
  rules:
    - host: one.insomnial.rest
      http:
        paths:
          - path: /v1/.*
            backend:
              service:
                name: insomnial-api-service-0
                port:
                  number: 80
...
kind: Ingress
metadata:
  name: insomnial-api-2
  annotations:
    kubernetes.io/ingress.class: "kong"
spec:
  rules:
    - host: two.insomnial.rest
      http:
        paths:
          - path: /v2/.*
            backend:
              service:
                name: insomnial-api-service-1
                port:
                  number: 80
            pathType: 'ImplementationSpecific'
```
...
info:
  x-kubernetes-ingress-metadata:
    name: insomnial-api                          # Kubernetes Ingress document name
