...
function _b64UrlToBigInt(s: string) {
  ...
  // @ts-ignore -- unfortunately, we must ignore here instead of the usual expect-error because this mondule is being used by two different builds (`insomnial` and `insomnial-send-request`) and in one of them this line is an error (`insomnial-send-request`) and the other it is not ()`insomnial`).
