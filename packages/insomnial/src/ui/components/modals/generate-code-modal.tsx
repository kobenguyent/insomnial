...
export const GenerateCodeModal = forwardRef<GenerateCodeModalHandle, Props>((props, ref) => {
  ...
  try {
    storedTarget = JSON.parse(window.localStorage.getItem('insomnial::generateCode::target') || '') as HTTPSnippetTarget;
  } catch (error) {}

  try {
    storedClient = JSON.parse(window.localStorage.getItem('insomnial::generateCode::client') || '') as HTTPSnippetClient;
  } catch (error) {}
  ...
  const generateCode = useCallback(async (request: Request, target?: HTTPSnippetTarget, client?: HTTPSnippetClient) => {
    ...
    window.localStorage.setItem('insomnial::generateCode::client', JSON.stringify(clientOrFallback));
    window.localStorage.setItem('insomnial::generateCode::target', JSON.stringify(targetOrFallback));
  }, [props.environmentId]);
