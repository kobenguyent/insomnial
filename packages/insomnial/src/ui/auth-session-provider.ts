...
encodeBase64(sessionKeyPair.publicKey).then(res => {
  window.localStorage.setItem('insomnial.publicKey', res);
});
encodeBase64(sessionKeyPair.secretKey).then(res => {
  window.localStorage.setItem('insomnial.secretKey', res);
});
/**
...
export async function submitAuthCode(code: string) {
  try {
    ...
    const publicKey = await decodeBase64(window.localStorage.getItem('insomnial.publicKey') || '');
    const secretKey = await decodeBase64(window.localStorage.getItem('insomnial.secretKey') || '');
...
export async function getLoginUrl() {
  const publicKey = window.localStorage.getItem('insomnial.publicKey');
  if (!publicKey) {
