...
export const randomDataPath = () => path.join(os.tmpdir(), 'insomnial-smoke-test', `${uuidv4()}`);
...
const pathLookup: Record<string, string> = {
  ...
  linux: path.join('linux-unpacked', 'insomnial'),
};
export const cwd = path.resolve(__dirname, '..', '..', 'insomnial');
